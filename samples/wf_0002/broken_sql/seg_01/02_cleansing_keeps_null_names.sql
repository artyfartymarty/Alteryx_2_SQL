...
-- The mistake: tool 2's `replace NULL strings with blank` option is not translated, so a NULL
-- NAME or CITY stays NULL instead of becoming the empty string the contract declares NOT NULL
-- for this stream -- and a NULL would then behave differently in every downstream comparison.
