...
-- The mistake: the Cross Tab's conditional sums are written with a zero default instead of a
-- NULL one, the reflex of anyone who has ever built a pivot for a report. Alteryx leaves a
-- group/warehouse combination with no rows NULL, and the difference is invisible until a
-- consumer adds the columns up.
...
BEGIN
  ...
  t4_cross_tab AS (
      SELECT
          SKU,
          FAMILY,
          CAST(SUM(IFF(WAREHOUSE = 'EAST', QTY, 0)) AS FLOAT)   AS EAST,
          CAST(SUM(IFF(WAREHOUSE = 'NORTH', QTY, 0)) AS FLOAT)  AS NORTH,
          CAST(SUM(IFF(WAREHOUSE = 'WEST', QTY, 0)) AS FLOAT)   AS WEST
      FROM t3_regex
      GROUP BY SKU, FAMILY
