...
-- The mistake: the macro's RegEx is translated as if CopyUnmatched were False, which is the
-- shape a translator reaches for when writing "replace when it matches" from scratch. The
-- flag is True, so a SKU the pattern does not match must be kept as it was; here it becomes
-- NULL and the row loses its identity entirely.
...
BEGIN
  ...
  t2_macro_m2_regex AS (
      SELECT
          IFF(REGEXP_LIKE(SKU, '^\\s*([A-Za-z]+)[- ]?(\\d+)\\s*$', 'i'),
              REGEXP_REPLACE(SKU, '^\\s*([A-Za-z]+)[- ]?(\\d+)\\s*$', '\\1-\\2', 1, 0, 'i'),
              NULL) AS SKU,
          WAREHOUSE,
          QTY,
          NOTE
