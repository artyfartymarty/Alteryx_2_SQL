...
-- The mistake: tool 3's `%d/%m/%Y` is translated as the month-first 'MM/DD/YYYY', the format
-- a reader who did not check the Alteryx configuration would reach for. Every unambiguous
-- day swaps with its month, and every day past the twelfth becomes NULL instead of a date --
-- which then changes the Sort, the Unique and the period-end flag in seg_02 as well.
...
BEGIN
  ...
  t3_datetime AS (
      SELECT
          ...
          REGION,
          ENTRY_ID,
          TRY_TO_TIMESTAMP_NTZ(POSTED, 'MM/DD/YYYY') AS POSTED_DT
      FROM t2_filter_t
  )
  SELECT
      ACCT,
