...
-- The mistake: Output tool 10's PreSQL is not translated at all. The DELETE is what clears the
-- pre-2020 history before the load, so without it the target keeps a period the Alteryx run
-- removes -- a row that is present in the migrated table and absent from the golden output.
