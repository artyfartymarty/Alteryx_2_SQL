...
-- The mistake: Output tool 10's PostSQL is not translated at all -- a reader who treated the
-- <PostSQL> element as connection housekeeping rather than part of the write. LOADED_FLAG is
-- not one of the columns the Summarize produces, so every row this run inserts keeps the NULL
-- the MERGE left it with instead of being marked 'Y'.
