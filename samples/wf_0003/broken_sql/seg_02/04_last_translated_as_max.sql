...
-- The mistake: tool 9's Summarize `Last` is translated as a plain `MAX(RUN_BAL)` instead of
-- `MAX_BY(RUN_BAL, RECORD_ID)`. Neither dialect has an ordered aggregate for "the group's last
-- row in incoming order", and reaching for `MAX` instead of `MAX_BY` is the natural mistake a
-- translator makes -- CLOSING_BAL becomes the group's largest running balance instead of its
-- actual last value. The two only disagree once a group's running balance peaks before its last
-- row, which is exactly what the `normal` set's `(4000, 2026-08)` group does once `ENTRY_ID` 14
-- is in it (fix round 1; see samples/wf_0003/README.md).
...
BEGIN
  ...
  -- MERGE does.
  -- ===========================================================================================
  DELETE FROM IDENTIFIER(:GL_SUMMARY_TGT)
