...
-- The mistake: tool 3's False branch is written as `WHERE REGION = 'WEST'`, the naive mirror
-- of the True branch. Alteryx sends rows whose Filter expression evaluates to NULL to the
-- False anchor, so every NULL-REGION order is silently lost from Output tool 8's stream 3_F.
