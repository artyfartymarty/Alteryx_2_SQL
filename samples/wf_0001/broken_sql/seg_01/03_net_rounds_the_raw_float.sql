...
-- The mistake: tool 4's Round is applied to the raw FLOAT column AMOUNT and the ×0.9 is then
-- left in FLOAT, instead of rounding the NUMBER product. Every discounted order keeps its
-- third decimal, so NET -- and the TOTAL_NET the Summarize adds up -- drifts by a fraction of
-- a cent from the half-away-from-zero result Alteryx computes on exact decimals.
