...
-- The mistake: tool 3's True branch over-corrects for NULLs with `OR REGION IS NULL`. Alteryx
-- treats a NULL Filter expression as false, so those rows belong on the False anchor only;
-- here they also reach the Summarize and open a group whose REGION is NULL.
