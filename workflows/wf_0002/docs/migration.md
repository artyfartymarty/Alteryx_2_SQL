...
as the touchpoint key. The procedures never name any of these tables: each name is built once with
`LET <LOGICAL>_SRC VARCHAR := SRC_DB || '.' || SRC_SCHEMA || '.<LOGICAL>';` (or `<LOGICAL>_TGT`
from `TGT_DB` and `TGT_SCHEMA`, named without a colon inside the LET), sources are read as `IDENTIFIER(:<LOGICAL>_SRC)` and the target
written as `IDENTIFIER(:<LOGICAL>_TGT)` (Snowflake's documented `IDENTIFIER` form, which takes one value, never an expression; chosen because the expression form is not in Snowflake's grammar, it has not run on a real account yet, and the first real-account run confirms it).
...
- `segments/seg_01/proc.sql`, `segments/seg_02/proc.sql` and `segments/seg_03/proc.sql` each create
  one segment procedure (`MIG_WORK.WF0002_SEG_01` … `MIG_WORK.WF0002_SEG_03`): `LANGUAGE SQL`,
  contract C4's five parameters, `EXECUTE AS CALLER`. Each is the whole translation of its segment.
- `procs/master.sql` creates `MIG_WORK.WF0002_MASTER`, generated by the orchestrator from
  `segments/order.json`. It takes the same five arguments, is `EXECUTE AS CALLER` too, and calls the
  segment procedures wave by wave: `seg_01` and `seg_02` (wave 1); then `seg_03` (wave 2). Deploy it
  after the procedures it calls.
