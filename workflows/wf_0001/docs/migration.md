...
The procedure never names any of these tables. Each name is built once at the top of the body,
`LET <LOGICAL>_SRC VARCHAR := SRC_DB || '.' || SRC_SCHEMA || '.<LOGICAL>';` for a source and
`LET <LOGICAL>_TGT VARCHAR := TGT_DB || '.' || TGT_SCHEMA || '.<LOGICAL>';` for a target; sources
are then read as `IDENTIFIER(:<LOGICAL>_SRC)` and targets written as `IDENTIFIER(:<LOGICAL>_TGT)`
(Snowflake's documented `IDENTIFIER` form, which takes one value, never an expression; chosen because the expression form is not in Snowflake's grammar, it has not run on a real account yet, and the first real-account run confirms it), so the same body runs against the golden view schema in validation and against the real
...
- `segments/seg_01/proc.sql` creates `MIG_WORK.WF0001_SEG_01`, the whole translation: a `LANGUAGE
  SQL` procedure with contract C4's five parameters and `EXECUTE AS CALLER`.
- `procs/master.sql` creates `MIG_WORK.WF0001_MASTER`, generated by the orchestrator from
  `segments/order.json`. It takes the same five arguments, is `EXECUTE AS CALLER` too, and calls the
  segment procedures wave by wave: `seg_01` (wave 1). Deploy it after the procedures it calls.
