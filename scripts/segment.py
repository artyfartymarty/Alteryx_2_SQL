"""Deterministic DAG segmentation: `workflows/<id>/parsed/dag.json` -> `segments/` (program spec §7.2).
    ...
    python scripts/segment.py <wf_id> [--min-tools N] [--max-tools N] [--root .]

A segment becomes one Snowflake stored procedure, so the cuts below trade off translator context
size against segment count: Tool Containers and macro boundaries are the author's own grouping and
are respected first; everything else is merged or split to land each segment around
...
warnings -- an unsplittable oversized group or an ordering dependency crossing a macro is still a
valid (if imperfect) segmentation, and the analyzer agent reviews warnings, so they don't block the
pipeline. 1 is a domain failure: no valid segmentation could be produced at all, i.e. the group
graph itself has a cycle and step 7 cannot find a topological order for it. 2 is for usage errors:
bad arguments, or a workflow that hasn't been parsed yet, so there is nothing to segment.
...
def segment(dag: dict, *, min_tools: int = 15, max_tools: int = 40, formula_heavy_cap: int = 20) -> dict:
    ...
    lowest tool id, so the result is fully deterministic.
    """
    ...
    def is_hard(e: dict) -> bool:
        """A hard cut: the edge touches a macro node. Macros are always their own segment."""
        return nodes_by_id[e["src"]]["type"] == "macro" or nodes_by_id[e["dst"]]["type"] == "macro"
    ...
    for start in ordering_starts:
        ...
        while True:
            ...
            if pred_node["type"] == "macro":
                warnings.append(f"ordering dependency of tool {start} crosses macro {pred}")
                break
    ...
    def find_split(members):
        """The best non-protected bridge in `members`' induced subgraph, or None."""
        member_set = set(members)
        ...
        for idx, e in enumerate(internal):
            ...
            diff = abs(size_of(comp_a) - size_of(comp_b))
    ...
    while changed:
        ...
        for members in groups_list:
            ...
            if split is None:
                cap = max_tools if size_of(members) > max_tools else formula_heavy_cap
                warnings.append(f"segment {sorted(members, key=int)} stays above its size cap "
                                 f"({size_of(members)} > {cap}): no splittable bridge remains")
                next_groups.append(members)
                continue
            comp_a, comp_b = split
    ...
    return {"segments": segments, "order": order, "warnings": warnings,
            "params": {"min_tools": min_tools, "max_tools": max_tools, "formula_heavy_cap": formula_heavy_cap}}
...
def run(repo: Repo, wf_id: str, **overrides) -> dict:
    ...
    Reads `manifest.segmentation` for `min_tools`/`max_tools` (and, if present, `formula_heavy_cap`)
    when `overrides` doesn't supply them. Writes `segments/seg_NN/dag.json` (contract C7),
